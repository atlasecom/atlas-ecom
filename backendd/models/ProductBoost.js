...
productBoostSchema.index({ product: 1, status: 1 });
productBoostSchema.index({ seller: 1, status: 1 });
productBoostSchema.index({ status: 1, priority: -1, createdAt: -1 });
productBoostSchema.index({ 'stats.remainingClicks': 1, status: 1 });

// Middleware pour mettre à jour updatedAt
productBoostSchema.pre('save', function(next) {
  this.updatedAt = Date.now();
  next();
});

// Méthode pour enregistrer un clic
productBoostSchema.methods.recordClick = function(source, userAgent, ipAddress) {
...
productBoostSchema.methods.isActive = function() {
...
module.exports = mongoose.model('ProductBoost', productBoostSchema);
