...
const CheckoutSteps = ({ active }) => {
    ...
    console.log(active);
    return (
        <div className={`w-full flex justify-center ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
            <div className={`w-[90%] 800px:w-[50%] flex items-center flex-wrap ${isRTL ? 'flex-row-reverse' : ''}`}>
                <div className={`${styles.noramlFlex}`}>
                    <div className={`${styles.cart_button}`}>
                        <span className={`${styles.cart_button_text}`}>1. {t('checkoutSteps.shipping')}</span>
                    </div>
