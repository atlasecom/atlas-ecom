...
const VerifiedBadge = ({ size = 'sm', className = '' }) => {
  ...
  return (
    <div className={`${sizeClasses[size]} relative group ${className}`}>
      ...
      />
      {/* Hover effect */}
      <div className="absolute inset-0 bg-gradient-to-r from-blue-400/20 to-green-400/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200"></div>
    </div>
  );
};
